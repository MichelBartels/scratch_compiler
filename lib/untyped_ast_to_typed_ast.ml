      ...
      Argument
        ( ( match function_name with
          | Some name ->
              name
          | None ->
              failwith @@ "arguments can only be inside functions: " ^ a )
        , a )
  | Variable v ->
      Variable v
  | Literal l ->
      Type
        ( match l with
        | Primitive l ->
            Scratch_value.get_primitive_type l
        | List _ ->
            failwith "lists are not literals" )
...
let statement_function_map f sprites =
  let rec map_function f stmt =
    ...
    @ ( ( match stmt with
        ...
        | Untyped_ast.WhileNot (_, body) ->
            List.map (map_function f) body
        | Untyped_ast.Repeat (_, body) ->
            List.map (map_function f) body
        | _ ->
            [] )
      |> List.flatten )
...
let infer_var_type v e =
  statement_map
    (fun ?function_name -> function
      | SetVariable (v', e) when v' = v ->
          [output_type ?function_name e]
      | IncrVariable (v', _) when v' = v ->
          [Type Float]
      | AddToList (v', e) when v' = v ->
          [output_type ?function_name e]
      | SetIndex s when s.list = v ->
          [output_type ?function_name s.value]
      | _ ->
          [] )
    e
    ...
    (fun ?function_name -> function
      | FuncCall (fn, args) when fn = arg_fn -> (
        ...
        | Some e ->
            [output_type ?function_name e]
        | None ->
            failwith "function missing argument" )
      | _ ->
          [] )
    e
  |> InferredTypeSet.of_list
